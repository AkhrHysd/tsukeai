...
const SYSTEM_PROMPT = [
  ...
  [
    ...
    "Prefer concrete sensory images that are grounded in the source text.",
    "Let emotion and meaning linger beyond the literal words (余情 — yojō).",
    "Do NOT add a season, weather, sky, wind, flower, moon, or other natural image unless the source text clearly supports it.",
    "Use a seasonal word (季語 — kigo) only when it follows from the source text or parent post context.",
    "Juxtapose source-grounded images to create resonance rather than stating emotion directly (取り合わせ — toriawase).",
  ...
  [
    ...
    "なまえだけのこる [8: な-ま-え-だ-け-の-こ-る] ← jiari is valid within the 7±1 contract",
  ].join("\n"),
].join(" ");
...
const SYSTEM_PROMPT_KANJI = [
  ...
  [
    ...
    "5-7-5 input: ゆびさきに / のこるぬくもり / ことばなく",
    "5-7-5 output:",
    "指先に",
    "残るぬくもり",
    "言葉なく",
    "7-7 input: てがみをたたむ / まだいえぬまま",
    "7-7 output:",
    "手紙を畳む",
    "まだ言えぬまま",
  ].join("\n"),
