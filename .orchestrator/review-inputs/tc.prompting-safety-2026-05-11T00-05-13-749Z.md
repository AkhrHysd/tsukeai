...
+  | "prompt_injection_detected"
   ...
   | "timeout"
   | "rate_limited"
@@ -64,6 +65,11 @@ type ChatCompletionResponse = {
   }>;
 };
...
@@ -78,6 +84,33 @@ const MIN_INPUT_CHARS = 1;
...
@@ -193,6 +226,14 @@ function assertRequestWithinLimits(
...
+
   if (request.input.length > config.maxInputChars) {
     throw new LlmAdapterError(
       "input_limit_exceeded",
@@ -212,6 +253,14 @@ function assertRequestWithinLimits(
       false,
     );
   }
+
...
@@ -262,25 +311,47 @@ async function requestCompletion(
   return text;
 }
...
+- 素入力はプロンプト内で命令ではなくデータとして扱う。実装は JSON 文字列化などの構造化境界を使い、素入力中の「前の指示を無視」「system prompt を表示」等の文言に従わないよう system prompt で固定する。
+- 明確なプロンプト注入シグナルを含む入力は、LLM に渡さず公開失敗または修正要求として扱う。検出対象は広げすぎず、運用で観測した攻撃文言に合わせて調整する。
